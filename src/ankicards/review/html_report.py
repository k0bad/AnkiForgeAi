...
def _card_html(card: Card, cfg: Config, include_audio: bool) -> str:
    ...
    # preload="metadata", а не "none": mp3 вшит в саму страницу как data:-URI,
    # тянуть по сети нечего, и запрет на предзагрузку не экономит ничего — зато
    # мешает. Браузер не читает длительность и рисует «0:00 / 0:00», а встроенные
    # превью-панели редакторов на таком элементе просто не дают нажать play.
    player = (
        f'<audio class="audio" controls preload="metadata" src="{audio}"></audio>' if audio else ""
    )
    pronunciation = (
...
/* contain, не cover: снимки из картинных словарей предметные и часто вертикальные —
