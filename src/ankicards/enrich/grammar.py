...
def forms_complete(card: Card) -> bool:
    ...
    Не `bool(card.forms)`: источник может проставить существительному только род
    (в словарях он есть), и по «непусто» такая карточка выглядела бы готовой — склонение
...
async def enrich_grammar_batch(cards: list[Card]) -> list[Card]:
    ...
    for card in cards:
        ...
        if generated:
            ...
            # пришёл из источника, а не из догадки модели,
