...
def test_audio_is_not_told_to_skip_preloading(tmp_path: Path) -> None:
    """preload="none" имеет смысл для файла по сети — но mp3 вшит в саму страницу.

    Тянуть нечего, экономии нет, а браузер из-за запрета не читает длительность
    (рисует «0:00 / 0:00») и во встроенных превью-панелях не даёт нажать play.
    """
    page = _page(tmp_path)
